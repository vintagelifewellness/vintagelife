...
export default function OrderDetails({ data }) {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`/api/order/recycleorder/delete/${data._id}`);
  ...
  const handlerecycle = async () => {
    ...
    const confirmDelete = confirm("Are you sure you want to restore this Order?");
    if (!confirmDelete) return;

    setLoading(true);
    try {
      await axios.put(`/api/order/recycleorder/restore/${data._id}`);
      setSuccess("Order restored successfully.");
      location.reload();
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="flex justify-end gap-4 mt-4 max-w-[800px] mx-auto">
      <button
        type="button"
      ...
      >
        ...
        disabled={loading}
        className="bg-green-600 hover:bg-green-700 text-sm px-4 py-2 rounded text-white transition duration-200 disabled:opacity-50"
      >
        Restore Order
      </button>
