...
export default function CandFInfoCard() {
    ...
    // Form state tailored for C&F Authorized Person / Owner
    const [formData, setFormData] = useState({
        name: "", dob: "", mobileNo: "", fatherOrHusbandName: "",
        gender: "", role: "", maritalStatus: "",
    });

    useEffect(() => {
        const fetchCandFData = async () => {
            if (!session?.user?.email) return;
            setIsFetching(true);
            try {
                // UPDATE THIS URL: C&F ka data lane wali API
                ...
                if (response.data?._id) {
                    ...
                    const dobForInput = candfData.dob ? new Date(candfData.dob).toISOString().split('T')[0] : "";
                    setFormData({
                        name: candfData.name || "",
                        dob: dobForInput,
                        mobileNo: candfData.mobileNo || "",
                        fatherOrHusbandName: candfData.fatherOrHusbandName || "",
                        gender: candfData.gender || "",
                        role: candfData.role || "", // Updated from profession to role
                        maritalStatus: candfData.maritalStatus || "",
                    });
                }
            } catch (error) {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            // UPDATE THIS URL: C&F ko update karne wali API
            await axios.patch("/api/candf/update-candf", { id: data._id, ...formData });
            ...
            console.error("Failed to update C&F info:", error);
        } finally {
    ...
    if (isFetching) {
        return <CandFInfoCardSkeleton />;
    }

    // Definition for form fields tailored for C&F Contact Person
    const formFields = [
        { label: "Authorized Person Name", name: "name", type: "text", isRequired: true },
        ...
        // { label: "Designation / Role", name: "role", type: "select", options: ["", "Proprietor", "Partner", "Director", "Manager", "Other"] }, // Updated options
    ...
    return (
        <div>
            ...
            <div className="bg-white dark:bg-gray-900 p-4 sm:p-6 border border-gray-200 dark:border-gray-800  shadow-sm">
                ...
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    <InfoField label="Authorized Person Name" value={data?.name} isRequired />
                    <InfoField label="D.O.B." value={data?.dob ? new Date(data.dob).toLocaleDateString("en-GB") : ""} />
                    <InfoField label="Email address" value={data?.email} /> {/* Assume email comes from main data obj */}
                    ...
                    {/* <InfoField label="Designation / Role" value={data?.role} /> */}
