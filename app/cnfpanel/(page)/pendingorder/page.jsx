...
import { useSession } from "next-auth/react"; // 1. Session import kiya
import axios from "axios"; // 2. Axios import kiya

export default function PendingOrders() {
    const { data: session, status } = useSession(); // 3. Login User ka data liya
    ...
    useEffect(() => {
        // 4. Jab user login ho jaye, tabhi uske orders mangwao
        if (status === "authenticated" && session?.user?.dscode) {
            fetchOrders(session.user.dscode);
        }
    }, [session, status]);
    ...
    const fetchOrders = async (dscode) => {
        ...
        try {
            // 5. Nayi API call ki jo sirf is user ke orders degi
            const response = await axios.get(`/api/c&f/cnf/${dscode}`);
            const result = response.data;
            
            if (result.success) {
                // Backend se `recentOrders` aa raha hai, usme sirf pending wale filter karo (status === false)
                const pendingOnly = result.recentOrders.filter(order => order.status === false || order.status === "Pending");
    ...
    const applyFilter = () => {
        const filtered = allOrders.filter((order) => {
            // Check if order.date exists before splitting
    ...
    // Agar session load ho raha hai, toh loading screen dikhao
    if (status === "loading") {
    ...
    return (
        <div className="max-w-7xl mx-auto lg:p-6 p-3 bg-white dark:bg-gray-700 shadow-lg rounded-lg text-gray-700 dark:text-white">
            <h2 className="text-2xl font-semibold mb-4 text-center">My Pending Orders</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                <div>
                    <label className="block text-sm font-medium">Date From</label>
                    <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="border rounded-lg p-2 w-full bg-white dark:bg-gray-700 text-gray-700 dark:text-white" />
                </div>
                <div>
                    <label className="block text-sm font-medium">Date To</label>
                    <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="border rounded-lg p-2 w-full bg-white dark:bg-gray-700 text-gray-700 dark:text-white" />
                </div>
                <div>
                    <label className="block text-sm font-medium">Order No</label>
                    <input type="text" value={orderNoFilter} onChange={(e) => setOrderNoFilter(e.target.value)} className="border rounded-lg p-2 w-full bg-white dark:bg-gray-700 text-gray-700 dark:text-white" placeholder="Filter by order number" />
                </div>
                <div>
                    <label className="block text-sm font-medium">DsCode</label>
                    <input type="text" value={dscodeFilter} onChange={(e) => setDscodeFilter(e.target.value)} className="border rounded-lg p-2 w-full bg-white dark:bg-gray-700 text-gray-700 dark:text-white" placeholder="Filter by dscode" />
                </div>
            ...
            {loadingOrders ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse border border-gray-300 rounded-lg overflow-hidden min-w-[1000px]">
                        <thead>
                            <tr className="bg-gray-100 dark:bg-gray-600 text-sm md:text-base">
                                ...
                                <th className="border border-gray-300 px-2 md:px-4 py-2">C&F Name</th>
                            ...
                            {filteredOrders.length > 0 ? (
                                filteredOrders.map((order) => (
                                    <tr key={order._id} className="text-center bg-white dark:bg-gray-800 text-sm md:text-base">
                                        ...
                                        <td className="border border-gray-300 px-2 md:px-4 py-2">{order.cfName || "N/A"}</td>
                                        <td className="border border-gray-300 px-2 md:px-4 py-2">{order.dscode}</td>
                                        <td className="border border-gray-300 text-left px-2 md:px-4 py-2">{order.dsname}</td>
                                        ...
                                        <td className="border border-gray-300 px-2 md:px-4 py-2">{order.date ? new Date(order.date).toLocaleDateString("en-GB") : "N/A"}</td>
                                ...
                                <tr>
                                    <td colSpan="9" className="text-center p-4 text-gray-500">No Pending C&F orders found</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
            ...
            {selectedOrder && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-gray-800 w-full h-full p-6 overflow-auto relative">
                        <button onClick={closeModal} className="absolute top-4 right-4 text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white text-2xl">×</button>
