...
const AppSidebar = () => {
                        ...
                        {nav.subItems ? (
                            <>
                                <button
                                    ...
                                    className={`w-full flex items-center menu-item group rounded-md p-2 transition-colors ${isSubActive || isSubmenuOpen
                                ...
                                >
                                    ...
                                    {(isExpanded || isHovered || isMobileOpen) && (
                                        <ChevronDown
                                            className={`ml-auto w-4 h-4 shrink-0 transition-transform duration-200 ${isSubmenuOpen ? "rotate-180" : ""
                                                }`}
                                        />
                                    )}
                                </button>
                                ...
                                {(isExpanded || isHovered) && (
                                    <div
                                        className={`overflow-hidden transition-all duration-300 ease-in-out ${isSubmenuOpen ? "max-h-[800px] opacity-100 mt-1" : "max-h-0 opacity-0"
                                            }`}
                                    >
                                            ...
                                            {nav.subItems.map((sub) => (
                                                <li key={sub.path}>
                                                    <Link
                                                        ...
                                                        prefetch={false}
                                                        className={`block py-2 text-sm transition-all duration-200 rounded-md px-3 whitespace-nowrap ${isActive(sub.path) ? "font-semibold bgg textn textw" : "hover:text-gray-300"
                                                            }`}
                                                    >
                                                        {sub.name}
                                                    </Link>
                            ...
                            nav.path && (
                                <Link
                                    ...
                                    onClick={handleLinkClick}
                                    prefetch={false}
                                    className={`flex items-center p-2 menu-item group rounded-md transition-colors ${isActive(nav.path) ? "bgg textn font-semibold textw" : "hbgb"
    ...
    return (
        <aside
            className={`fixed mt-16 flex flex-col lg:mt-0 top-0 px-4 py-4 left-0 bgn textw border-r h-screen transition-all duration-300 ease-in-out z-50 ${isExpanded || isMobileOpen ? "w-[290px]" : isHovered ? "w-[290px]" : "w-[80px]"
        ...
        >
            ...
            <div className="mb-4">
                <div className={`flex items-center bg-white border border-white/10 rounded-lg overflow-hidden transition-all duration-200 focus-within:border-white/30 focus-within:bg-white/10 ${isExpanded || isHovered || isMobileOpen ? " " : " justify-center cursor-pointer"
                    }`}>
                    <Search size={18} className="text-gray-400 shrink-0 ml-3" />
                    {(isExpanded || isHovered || isMobileOpen) && (
                        <input
                            type="text"
                            ...
                            className="bg-transparent border-none outline-none text-sm px-1 py-2  text-black  ml-2 w-full placeholder:text-gray-500"
                        />
                    )}
                </div>
            </div>
