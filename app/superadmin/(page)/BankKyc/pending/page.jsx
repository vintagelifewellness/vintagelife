...
export default function Page() {
    ...
    const fetchData = async (page = 1, dscode = "") => {
        try {
            setLoading(true);
            const res = await axios.post("/api/kyc/pendingkyc/bank", { page, dscode });
    ...
    return (
        <div className="">
            <h1 className="text-xl md:text-2xl font-bold mb-4">Pending Bank KYC Users</h1>
                        ...
                        <table className="min-w-full border text-sm">
                            <thead className="bg-gray-100">
                                <tr>
                                    ...
                                    <th className="border px-3 py-2 text-left">A/C Number</th>
                                    <th className="border px-3 py-2 text-left">Ifsc Code</th>
                                    <th className="border px-3 py-2 text-left">Bank Name</th>
                                    <th className="border px-3 py-2 text-left">Bank Kyc Proof</th>
                            ...
                            <tbody>
                                {data.length > 0 ? (
                                    data.map((user, index) => (
                                        <tr key={user._id} className="odd:bg-gray-50 even:bg-slate-50">
                                            <td className="border px-3 py-2">{(page - 1) * 20 + index + 1}</td>
                                            <td className="border px-3 py-2">{user.dscode}</td>
                                            <td className="border px-3 py-2">{user.name}</td>
                                            <td className="border px-3 py-2">{user.acnumber || "-"}</td>
                                            <td className="border px-3 py-2">{user.ifscCode || "-"}</td>
                                            <td className="border px-3 py-2">{user.bankName || "-"}</td>
                                            <td className="border px-3 py-2">
                                                {user.bankimage ? (
                                                    <div className="w-12 h-12 overflow-hidden border border-gray-300">
                                                        <Link href={user.bankimage} target="_blank">
                                                            <Image
                                                                src={user.bankimage}
                                                                alt="Bank Image"
                                                                width={48}
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        const updated = data.map(u =>
                                                            u._id === user._id ? { ...u, bankresn: e.target.value, isCommentChanged: true } : u
                                                        );
                                                        setData(updated);
                                                    }}
                                                />
                                            ...
                                            <td className="border px-3 py-2">
                                                <Link href={`/superadmin/kycupdate/bankkyc/${user.email}`} className=" bgn px-2 rounded font-semibold text-white">
                                                    Edit
                                                </Link>
                                            </td>
                                            <td className="border px-3 py-2">
            ...
            {showModal && selectedUser && (
                <div className="fixed inset-0 bgn flex items-center justify-center z-50 p-4">
