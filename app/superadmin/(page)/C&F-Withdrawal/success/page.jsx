...
export default function CandFSuccessReport() {
  ...
  const fetchData = async (page = 1) => {
    try {
      const params = new URLSearchParams({
        page: page.toString(),
        limit: '10',
        status: 'true', // 👈 Sirf Success wale records mangne ke liye
      });
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'C&F Success Report')
    XLSX.writeFile(workbook, 'CandF_Success_Report.xlsx')
  }
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className='text-2xl font-bold text-green-700 uppercase underline'>Success C&F Points Report</h1>
      ...
      <div className="overflow-auto rounded-xl border border-gray-300 shadow-md">
        {data.length === 0 ? (
          <div className="text-center py-10 text-gray-500">No success records found</div>
        ) : (
