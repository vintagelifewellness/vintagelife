...
export default function Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get("/api/c&f/fetchalldeactive/user", { params });
    ...
    <div className="">
      <div className=" mx-auto">
        <header className="mb-2 text-center">
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-800 dark:text-white">Padding C&F Master Registry</h1>
        </header>
    ...
    <tbody>
      {users.map((user, index) => (
        <tr key={user.dscode} className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
          ...
          <td className="px-6 py-4 hidden sm:table-cell">{user.name}</td>
          ...
          <td className="px-6 py-4">
            <span className={`px-3 py-1 text-xs font-semibold rounded-full ${user.Cnftype === "1"
              ...
              {/* FIXED: user.usertype ki jagah user.Cnftype aayega jaisa active me tha */}
          ...
          <td className="px-6 py-4 text-right">
            <Link href={`/superadmin/C&F/PendingUser/${user.email}`} className="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 font-medium">
              <Eye size={14} />
              <span>View</span>
            </Link>
          </td>
