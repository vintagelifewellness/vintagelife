...
export default function CandFPointsClosingPage() {
  ...
  const fetchData = async (page = 1) => {
    try {
      const res = await fetch(`/api/candf/get-candf-points-closing?page=${page}&limit=10`)
  ...
  const handleConfirm = async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/candf/candf-points-closing', { method: 'POST' })
      const result = await res.json()
  ...
  const handleExport = () => {
    ...
    const formatted = unpaid.map(item => ({
      ...
      IFSC: item.ifscCode,
      Bank: item.bankName,
      'Last Month Points': item.lastmatchpoint, // Naya Column C&F ke liye
      'Used Points': item.usepoint, // Naya Column C&F ke liye
      'Difference (Pay Amount)': item.payamount, // Yehi unka pay amount hoga
      Date: item.date
    }))
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      {/* Naya H1 Tag jo tune bola tha */}
      <div className="border-b pb-4 mt-2">
        <h1 className="text-2xl font-bold text-gray-800 uppercase">C&F Points Closing</h1>
          ...
          <table className="min-w-full text-sm text-left">
            <thead className="bg-gray-100 text-gray-700">
              <tr>
                ...
                <th className="p-3 border">Last Month Pts</th> {/* Changed for C&F */}
                <th className="p-3 border text-blue-600">Used Pts</th> {/* Changed for C&F */}
                <th className="p-3 border font-bold text-green-600">Difference (Pay)</th> {/* Changed for C&F */}
            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  {/* Values for C&F Logic */}
                  <td className="p-3 border">{item.lastmatchpoint || 0}</td>
                  <td className="p-3 border font-semibold text-blue-600">{item.usepoint || 0}</td>
                  <td className="p-3 border font-bold text-green-600">{item.payamount}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-xl shadow-2xl max-w-sm w-full text-center animate-fade-in">
            ...
            <p className="text-gray-600 mb-6">This will generate C&F closing based on Used and Last Month points. Do you want to continue?</p>
