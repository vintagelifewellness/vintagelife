...
export default function Page() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await axios.get("/api/user/fetchalldeactive/user", { params });
        ...
        <tbody>
            {users.map((user) => (
                <tr key={user.dscode} className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                        <Link href={`/superadmin/Report/allreport/deactiveuser/${user.dscode}?email=${encodeURIComponent(user.email)}`} className="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 font-medium">
                            <Eye size={14} />
                            <span>View</span>
                        </Link>
                    </td>
