...
export default function DeliveredOrder() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        const response = await fetch(`/api/order/delivereduser/${data}`);
  ...
  const applyFilter = () => {
    const filtered = allOrders.filter((order) => {
      const orderDate = order.updatedAt.split("T")[0]; // Extract only the YYYY-MM-DD part
  ...
  const exportToExcel = () => {
    ...
    saveAs(data, "deliveredOrder.xlsx");
  };
  ...
  return (
    <div className="mx-auto lg:p-6 p-2 bg-white dark:bg-gray-700 shadow-lg  text-gray-700 dark:text-white">
      <h2 className="text-2xl font-semibold mb-4 text-center">My Delivered Order List</h2>
            ...
            {filteredOrders.length > 0 ? (
              filteredOrders.map((order) => (
                <tr key={order._id} className="text-center bg-white dark:bg-gray-800">
                  ...
                  <td className="border border-gray-300 px-4 py-2">{new Date(order.updatedAt).toLocaleDateString("en-GB")}</td>
                  <td className="border border-gray-300 px-4 py-2">{order.deliver ? "Delivered" : "Pending"}</td>
