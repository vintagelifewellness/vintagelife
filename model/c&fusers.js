...
const CnfSchema = new Schema(
  {
    cfName: { type: String, unique: true },
    ...
    Availablepoint: { type: Number, default: 0 },
    Usepoint: { type: Number, default: 0 },
    Lastmatchpoint: { type: Number, default: 0 },
    ...
    Cnftype: { type: String, enum: ["0", "3",], default: "0", required: true },
    defaultdata: { type: String, required: true, default: "Cnf" },
  },
  { timestamps: true }
);
...
CnfSchema.pre("save", async function (next) {
  const doc = this;
  ...
  try {
    ...
    while (!isUnique && attempts < 10) { // max 10 tries to prevent infinite loop
      ...
      const existingCnf = await this.constructor.findOne({ dscode: generatedCode }).lean();
      if (!existingCnf) {
        isUnique = true;
      }
    }
...
const CnfModel = mongoose.models.cnf3 || mongoose.model("cnf3", CnfSchema);

export default CnfModel;